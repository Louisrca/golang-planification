...
func DeleteService(db *sql.DB, id string) (model.Service, error) {
    var service model.Service
    err := db.QueryRow("SELECT id, name, price, duration, category_id, hair_salon_id FROM service WHERE id = ?", id).
        Scan(&service.ID, &service.Name, &service.Price, &service.Duration, &service.CategoryID, &service.HairSalonID)
    if err != nil {
